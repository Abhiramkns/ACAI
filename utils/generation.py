...
class Assistant:
    def __init__(self, config) -> None:
        # LLM interface
        self.pipe = pipeline("text-generation", model="meta-llama/Meta-Llama-3-8B-Instruct", device="cuda", torch_dtype=torch.float16, do_sample=False, temperature=None)
    ...
    def llm_response(self, query, img_url=None, response_img_url=None):
        ...
        if llm_prompt_pq_check is not None:
            ...
            answered_personal_question_flag, relation, entity = self.conversation.decode_pq_check(output)
            if answered_personal_question_flag:
                # ok, message = self.conversation.validate_current_answer(relation, entity)
                # if not ok:
                #     return message, None
                self.conversation.add_personal_info(relation, entity, img_url, prev_question, query)
        ...
        print('query: ', query)
        messages = self.conversation.get_history(query.strip())
        ...
        # self.conversation.add_assistant_response(str(res))
    
        # self.conversation.check_if_personal_answer(res, img_url, response_img_url)
